...
namespace HepKDE{
//--------------------------------------------------------------------------------------------------
KDEProducer1D::KDEProducer1D(const std::vector<double> * xvals,
...
//--------------------------------------------------------------------------------------------------
void KDEProducer1D::computeGlobalQuantities(const double hSF) {
    double sumW   = 0;
    double sumW2  = 0;
    double sumX   = 0;
    double sumX2  = 0;
...
//--------------------------------------------------------------------------------------------------
void KDEProducer1D::setupPilotKDE(const unsigned int nXBins, const double xMin, const double xMax) {
...
//--------------------------------------------------------------------------------------------------
void KDEProducer1D::setupPilotKDE(const unsigned int nXBins, const double* xBins) {
    //For speed, the pilot density will be computed
    //in a grid, set by the histogram bin centers
    pilotKDE.reset(getPDF("pilotKDE","; pilot KDE",nXBins,xBins));
}
//--------------------------------------------------------------------------------------------------
void KDEProducer1D::buildPilotKDE(const double trimFactor) {
...
//--------------------------------------------------------------------------------------------------
double KDEProducer1D::getADensity(const double x, double* weight) const {
    double dens = 0;
    double sumW2 =0;
    for(unsigned int iX = 0; iX < nDataPts; ++iX ){
...
double KDEProducer1D::geometricMean() const
{
    double sumTerms = 0;
...
//--------------------------------------------------------------------------------------------------
double KDEProducer1D::getDensity(const double x, double* weight) const {
    double dens = 0;
    double sumW2 =0;
    for(unsigned int iX = 0; iX < nDataPts; ++iX ){
        const double delt = (x - (*xvals)[iX]);
...
//--------------------------------------------------------------------------------------------------
TH1* KDEProducer1D::getPDF(const std::string& name, const std::string& title,
...
//--------------------------------------------------------------------------------------------------
TH1* KDEProducer1D::getAPDF(const std::string& name, const std::string& title,
...
//--------------------------------------------------------------------------------------------------
void KDEProducer1D::fillPDF(TH1* newHist,  const bool adaptive) const {
    double weight;
...
//--------------------------------------------------------------------------------------------------
TH1* KDEProducer1D::convToHist(const TH1 * hist) const {
...
//--------------------------------------------------------------------------------------------------
TH1* KDEProducer1D::getPilotPDF() const {
    TH1* oH = (TH1*)pilotKDE->Clone();
    return oH;
}
//--------------------------------------------------------------------------------------------------
TH1* KDEProducer1D::getABandwidths(const std::string& name, const std::string& title,
        const unsigned int nBins, const float xMin, const float xMax) const {
    TH1 * h = new TH1F(name.c_str(), title.c_str(), nBins,xMin,xMax);
    fillABandwidths(h);
    return h;
...
//--------------------------------------------------------------------------------------------------
void KDEProducer1D::fillABandwidths(TH1* newHist) const {
